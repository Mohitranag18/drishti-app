...
class NotificationService {
  ...
  async setupAutomaticReminders(userId) {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId },
        select: { wellness_reminders: true, weekly_summary: true }
      });
      ...
      // Create weekly check-in reminder if enabled
      if (user.weekly_summary) {
        // Check if weekly reminder already exists for next week
        const nextWeek = new Date();
        nextWeek.setDate(nextWeek.getDate() + 7);
        const weekAfter = new Date(nextWeek);
        weekAfter.setDate(weekAfter.getDate() + 1);

        const existingWeeklyReminder = await prisma.notification.findFirst({
          where: {
            user_id: userId,
            type: 'check_in',
            scheduled_for: {
        ...
        if (!existingWeeklyReminder) {
          await this.createWeeklySummaryReminder(userId);
          notifications.push('weekly_summary_reminder');
        }
      }
